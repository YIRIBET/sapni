...
function Orders() {
  ...
  const openModal = (order) => {
    if (order.campaign_status !== 1) {
      Swal.fire({
        icon: "warning",
        title: "Campaña inactiva",
        text: "No puedes editar órdenes de campañas inactivas",
      });
      return;
    }
  ...
  const handleDelete = async (orderId, campaignStatus) => {
    if (campaignStatus !== 1) {
      Swal.fire({
        icon: "warning",
        title: "Campaña inactiva",
        text: "No puedes eliminar órdenes de campañas inactivas",
      });
      return;
    }
      ...
      <div className="flex justify-end items-center mb-4 mr-4">
        <ExportPDFButton
          ...
          rows={filteredOrders.map((o) => ({
            ...o,
            _costo: `$${Number(o.contract_amount).toLocaleString("es-MX", {
                ...
                <tr key={order.id} className="border-t border-gray-200">
                  ...
                  <td className="py-2 px-4">
                    <span
                      className={`px-2 py-1 rounded text-xs ${
                        order.campaign_status === 1
                          ? "bg-green-100 rounded-full text-green-700"
                          : "bg-red-100 rounded-full text-red-600"
                      }`}
                    >
                      {order.campaign_status === 1
                        ? "Activa"
                  ...
                  <td className="py-2 px-4">
                    ...
                    <button
                      ...
                      onClick={() =>
                        handleDelete(order.id, order.campaign_status)
                      }
                    >
